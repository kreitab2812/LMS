...
import com.lms.quanlythuvien.services.library.BookManagementService; // Service Singleton
import com.lms.quanlythuvien.services.library.GoogleBooksService;   // Giả sử không phải Singleton, hoặc là Singleton nếu cần
...
public class BookManagementController implements Initializable {
    ...
    private void handleDeleteBookAction(ActionEvent event) {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            try {
                // Giả sử selectedBook.getIsbn13() trả về ISBN-13 của sách
                // vì deleteBookFromLibrary giờ đây nhận isbn13
                if (selectedBook == null || selectedBook.getIsbn13() == null || selectedBook.getIsbn13().isEmpty()) {
                    showAlert(Alert.AlertType.ERROR, "Lỗi", "Không có sách nào được chọn hoặc sách không có ISBN hợp lệ.");
                    return;
                }

                // Gọi phương thức delete, phương thức này có thể ném DeletionRestrictedException
                boolean deleted = bookManagementService.deleteBookFromLibrary(selectedBook.getIsbn13());

                if (deleted) {
                    loadBooksToTable(); // Tải lại danh sách
                    showAlert(Alert.AlertType.INFORMATION, "Thành công", "Đã xóa sách thành công!");
                } else {
                    // Trường hợp này ít xảy ra hơn nếu service ném exception cho các lỗi cụ thể
                    showAlert(Alert.AlertType.ERROR, "Lỗi", "Không thể xóa sách. Sách có thể không tìm thấy (dù đã được chọn).");
                }
            } catch (DeletionRestrictedException e) {
                // Bắt và hiển thị thông báo cụ thể từ exception
